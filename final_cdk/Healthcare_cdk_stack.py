...
class HealthcareCdkStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, stack_selection: str = "unknown", chat_tool_model: str = "us.amazon.nova-pro-v1:0", **kwargs) -> None:
        ...
        healthcare_collection_name = f"hc-{name_key}-col"
        # Create Healthcare Collection
        healthcare_collection = opensearch.CfnCollection(
            self, "HealthcareKBCollection",
            name=healthcare_collection_name,
            type="VECTORSEARCH"
        )

        # Create security policies for healthcare collection
        healthcare_encryption_policy = opensearch.CfnSecurityPolicy(
            self, "HealthcareKBSecurityPolicy",
            name=f"hc-{name_key}-encrypt",
            type="encryption",
            policy=json.dumps({
                "Rules": [{
                    "ResourceType": "collection",
                    "Resource": [f"collection/{healthcare_collection_name}"]
                }],
                "AWSOwnedKey": True
            })
        )

        healthcare_network_policy = opensearch.CfnSecurityPolicy(
            self, "HealthcareKBNetworkPolicy",
            name=f"hc-{name_key}-network",
            type="network",
            policy=json.dumps([{
                "Rules": [{
                    "ResourceType": "collection",
        ...
        healthcare_collection.add_dependency(healthcare_encryption_policy)
        healthcare_collection.add_dependency(healthcare_network_policy)
                    ...
                    statements=[
                        iam.PolicyStatement(
                            ...
                            resources=[
                                ...
                                f"arn:aws:aoss:{self.region}:{self.account}:collection/{healthcare_collection_name}",
                                f"arn:aws:aoss:{self.region}:{self.account}:index/{healthcare_collection_name}/*"
                            ]
                        )
                    ]
                ),
        ...
        healthcare_data_access_policy = opensearch.CfnAccessPolicy(
            self, "HealthcareKBDataAccessPolicy",
            name=f"hc-{name_key}-access",
            type="data",
            policy=json.dumps([{
                "Rules": [{
                    "ResourceType": "collection",
                    ...
                    "ResourceType": "index",
                    "Resource": [f"index/{healthcare_collection_name}/*"],
                    "Permission": [
                        "aoss:CreateIndex",
                        "aoss:DeleteIndex",
                        "aoss:UpdateIndex",
                ...
                "Description": f"Data access policy for {healthcare_collection_name}"
            }])
        )
        ...
        healthcare_index_name = f"hc-{name_key}-idx"

        healthcare_index_creator_function = lambda_.Function(
            self, "HealthcareIndexCreatorFunction",
            ...
            layers=[
                lambda_.LayerVersion(
                    self, "HealthcareOpenSearchPyLayer",
                    code=lambda_.Code.from_asset(str(layers_dir / "opensearchpy.zip")),
                    compatible_runtimes=[lambda_.Runtime.PYTHON_3_9],
                    description="OpenSearch Python client layer for Healthcare"
                ),
                lambda_.LayerVersion(
                    self, "HealthcareAWS4AuthLayer",
                    code=lambda_.Code.from_asset(str(layers_dir / "aws4auth.zip")),
                    compatible_runtimes=[lambda_.Runtime.PYTHON_3_9],
                    description="AWS4Auth layer for Healthcare"
                )
            ],
        ...
        healthcare_index_creator = CustomResource(
            self, "HealthcareIndexCreator",
            service_token=healthcare_provider.service_token,
            properties={
                "index_name": healthcare_index_name,
                "dimension": 1024,
                "method": "hnsw",
        ...
        healthcare_kb = self.create_kb(
            f"genaifoundryhealthcare-{name_key}",
            f"s3://{s3_bucket_name}/kb/healthcare/",
            model_arn,
            bedrock_kb_role.role_arn,
            "kb/healthcare",
            healthcare_index_name,
            healthcare_index_creator_function,
    ...
    'sudo su - ubuntu -c "/home/ubuntu/voice_bot.sh" > /var/log/voice_bot.log 2>&1'
        ...
        env_vars = {
            ...
            "KB_ID": healthcare_kb.attr_knowledge_base_id,  # Healthcare KB ID
            ...
            "TAVILY_API_KEY":"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",
        ...
        lambda_function = lambda_.Function(
            ...
            runtime=lambda_.Runtime.PYTHON_3_9,
            handler="healthcare.lambda_handler",
