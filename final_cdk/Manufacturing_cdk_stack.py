...
class ManufacturingCdkStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, stack_selection: str = "unknown", chat_tool_model: str = "us.amazon.nova-pro-v1:0", **kwargs) -> None:
        ...
        print(f"🏗️ Building Manufacturing Stack with selection: {self.stack_selection}")
        ...
        manufacturing_collection_name = f"man-{name_key}-col"
        # Create Manufacturing Collection
        manufacturing_collection = opensearch.CfnCollection(
            self, "ManufacturingKBCollection",
            name=manufacturing_collection_name,
            type="VECTORSEARCH"
        )

        # Create security policies for manufacturing collection
        manufacturing_encryption_policy = opensearch.CfnSecurityPolicy(
            self, "ManufacturingKBSecurityPolicy",
            name=f"man-{name_key}-encrypt",
            type="encryption",
            policy=json.dumps({
                "Rules": [{
                    "ResourceType": "collection",
                    "Resource": [f"collection/{manufacturing_collection_name}"]
                }],
                "AWSOwnedKey": True
            })
        )

        manufacturing_network_policy = opensearch.CfnSecurityPolicy(
            self, "ManufacturingKBNetworkPolicy",
            name=f"man-{name_key}-network",
            type="network",
            policy=json.dumps([{
                "Rules": [{
                    "ResourceType": "collection",
        ...
        manufacturing_collection.add_dependency(manufacturing_encryption_policy)
        manufacturing_collection.add_dependency(manufacturing_network_policy)
                    ...
                    statements=[
                        iam.PolicyStatement(
                            ...
                            resources=[
                                ...
                                f"arn:aws:aoss:{self.region}:{self.account}:collection/{manufacturing_collection_name}",
                                f"arn:aws:aoss:{self.region}:{self.account}:index/{manufacturing_collection_name}/*"
                            ]
                        )
                    ]
                ),
        ...
        manufacturing_data_access_policy = opensearch.CfnAccessPolicy(
            self, "ManufacturingKBDataAccessPolicy",
            name=f"man-{name_key}-access",
            type="data",
            policy=json.dumps([{
                "Rules": [{
                    "ResourceType": "collection",
                    ...
                    "ResourceType": "index",
                    "Resource": [f"index/{manufacturing_collection_name}/*"],
                    "Permission": [
                        "aoss:CreateIndex",
                        "aoss:DeleteIndex",
                        "aoss:UpdateIndex",
                ...
                "Description": f"Data access policy for {manufacturing_collection_name}"
            }])
        )

        # Add dependencies for data access policy
        manufacturing_collection.add_dependency(manufacturing_data_access_policy)
        ...
        manufacturing_index_name = f"man-{name_key}-idx"

        manufacturing_index_creator_function = lambda_.Function(
            self, "ManufacturingIndexCreatorFunction",
            runtime=lambda_.Runtime.PYTHON_3_9,
            handler="index_creator.lambda_handler",
            role=lambda_role,
            timeout=Duration.minutes(10),
            environment={
                "OPENSEARCH_ENDPOINT": manufacturing_collection.attr_collection_endpoint,
                "COLLECTION_NAME": manufacturing_collection_name,
                "INDEX_NAME": manufacturing_index_name,
                "chat_tool_model": self.chat_tool_model
            },
            layers=[
                lambda_.LayerVersion(
                    self, "ManufacturingOpenSearchPyLayer",
                    code=lambda_.Code.from_asset(str(layers_dir / "opensearchpy.zip")),
                    compatible_runtimes=[lambda_.Runtime.PYTHON_3_9],
                    description="OpenSearch Python client layer for Manufacturing"
                ),
                lambda_.LayerVersion(
                    self, "ManufacturingAWS4AuthLayer",
                    code=lambda_.Code.from_asset(str(layers_dir / "aws4auth.zip")),
                    compatible_runtimes=[lambda_.Runtime.PYTHON_3_9],
                    description="AWS4Auth layer for Manufacturing"
                )
            ],
        ...
        manufacturing_index_waiter_function = lambda_.Function(
            self, "ManufacturingIndexWaiterFunction",
            runtime=lambda_.Runtime.PYTHON_3_9,
            ...
            layers=[
                lambda_.LayerVersion(
                    self, "ManufacturingWaiterOpenSearchPyLayer",
                    code=lambda_.Code.from_asset(str(layers_dir / "opensearchpy.zip")),
                    compatible_runtimes=[lambda_.Runtime.PYTHON_3_9],
                    description="OpenSearch Python client layer for Manufacturing Index Waiter"
                ),
                lambda_.LayerVersion(
                    self, "ManufacturingWaiterAWS4AuthLayer",
                    code=lambda_.Code.from_asset(str(layers_dir / "aws4auth.zip")),
                    compatible_runtimes=[lambda_.Runtime.PYTHON_3_9],
                    description="AWS4Auth layer for Manufacturing Index Waiter"
                )
            ],
            code=lambda_.Code.from_asset(str(lambda_dir))
        )
        ...
        manufacturing_index_creator_function.node.add_dependency(manufacturing_collection)
        # Add dependency to ensure data access policy is applied before Lambda runs
        manufacturing_index_creator_function.node.add_dependency(manufacturing_data_access_policy)

        # Add dependencies for index waiter function
        manufacturing_index_waiter_function.node.add_dependency(manufacturing_collection)
        manufacturing_index_waiter_function.node.add_dependency(manufacturing_data_access_policy)

        # Create providers for each collection
        manufacturing_provider = cr.Provider(
            self, "ManufacturingInitProvider",
            on_event_handler=manufacturing_index_creator_function
        )

        # Create provider for index waiter
        manufacturing_waiter_provider = cr.Provider(
            self, "ManufacturingWaiterProvider",
            on_event_handler=manufacturing_index_waiter_function
        )

        # Create custom resource to create manufacturing index
        manufacturing_index_creator = CustomResource(
            self, "ManufacturingIndexCreator",
            service_token=manufacturing_provider.service_token,
            properties={
                "index_name": manufacturing_index_name,
                "dimension": 1024,
                "method": "hnsw",
        ...
        # Create custom resource to wait for manufacturing index readiness
        manufacturing_index_waiter = CustomResource(
            self, "ManufacturingIndexWaiter",
            service_token=manufacturing_waiter_provider.service_token,
            properties={
                "index_name": manufacturing_index_name,
        ...
        manufacturing_index_creator.node.add_dependency(manufacturing_collection)
        # Add dependency to ensure Lambda function is ready before index creation
        manufacturing_index_creator.node.add_dependency(manufacturing_index_creator_function)

        # Add dependencies for index waiter
        manufacturing_index_waiter.node.add_dependency(manufacturing_index_creator)
        manufacturing_index_waiter.node.add_dependency(manufacturing_index_waiter_function)
        manufacturing_index_waiter.node.add_dependency(manufacturing_waiter_provider)

        # Create both knowledge bases - POSITIONED LAST IN THE FLOW

        manufacturing_kb = self.create_kb(
            f"genaifoundrymanufacturing-{name_key}",
            f"s3://{s3_bucket_name}/kb/manafacturing/",
            model_arn,
            bedrock_kb_role.role_arn,
            "kb/manafacturing",
            manufacturing_index_name,
            manufacturing_index_creator_function,
            manufacturing_index_creator,
            manufacturing_provider,
            manufacturing_collection.attr_arn,
            manufacturing_data_access_policy,
            manufacturing_index_waiter,
            manufacturing_index_waiter_function,
            manufacturing_waiter_provider
        )

        # Dependencies are now handled inside the create_kb function
        manufacturing_kb.node.add_dependency(bedrock_kb_role)
        ...
        auto_sync_function = lambda_.Function(
            ...
            environment={
                "MANUFACTURING_KB_ID": manufacturing_kb.attr_knowledge_base_id,
                "MANUFACTURING_DS_ID": manufacturing_kb.data_source_id,
                "chat_tool_model": self.chat_tool_model
            },
            code=lambda_.Code.from_asset(str(lambda_dir))
        )
        ...
        initial_sync_function.node.add_dependency(manufacturing_kb)
        ...
        initial_sync.node.add_dependency(manufacturing_kb)
        ...
        try:
            ...
            try:
                ...
                self.data_bucket.add_event_notification(
                    ...
                    s3.NotificationKeyFilter(prefix="kb/manafacturing/")
                )
    ...
    roles=[ec2_role.role_name]
        ...
        ec2_instance.add_user_data(
            ...
            "until ! sudo fuser /var/lib/dpkg/lock-frontend /var/lib/dpkg/lock /var/lib/apt/lists/lock >/dev/null 2>&1 && ! pgrep -x unattended-upgr >/dev/null 2>&1 && ! pgrep -x apt-get >/dev/null 2>&1 && ! pgrep -x apt >/dev/null 2>&1; do echo 'waiting for apt lock...'; sleep 10; done",
            "sudo dpkg --configure -a || true",
            "sudo apt-get update -y",
            "sudo apt-get install -y apache2 awscli jq postgresql-client-14",
    ...
    'export NEEDRESTART_SUSPEND=1',
    '',
    '# Stop the auto-updater in case it re-triggered during this run',
    'sudo systemctl stop unattended-upgrades apt-daily.service apt-daily-upgrade.service || true',
    'sudo systemctl disable unattended-upgrades apt-daily.timer apt-daily-upgrade.timer || true',
    'sudo systemctl kill --kill-who=all unattended-upgrades 2>/dev/null || true',
    '',
    'wait_for_apt_lock() {',
    '  local i=0',
    '  while sudo fuser /var/lib/dpkg/lock-frontend >/dev/null 2>&1 || sudo fuser /var/lib/dpkg/lock >/dev/null 2>&1 || sudo fuser /var/lib/apt/lists/lock >/dev/null 2>&1 || pgrep -x unattended-upgr >/dev/null 2>&1 || pgrep -x apt-get >/dev/null 2>&1 || pgrep -x apt >/dev/null 2>&1; do',
    '    i=$((i+1))',
    '    if [ "$i" -gt 60 ]; then',
    ...
    'sudo su - ubuntu -c "/home/ubuntu/voice_bot.sh" > /var/log/voice_bot.log 2>&1'
        ...
        env_vars = {
            ...
            "MAN_KB_ID": manufacturing_kb.attr_knowledge_base_id,  # Manufacturing KB ID
        ...
        lambda_function = lambda_.Function(
            ...
            runtime=lambda_.Runtime.PYTHON_3_9,
            handler="manafacturing.lambda_handler",
      ...
      "#!/bin/bash",
        ...
        # CfnOutput(
        #     self,
        #     "ManufacturingOpenSearchCollectionArn",
        #     value=manufacturing_collection.attr_arn,
        #     description="ARN of the Manufacturing OpenSearch Serverless collection"
        # )

        # CfnOutput(
        #     self,
        #     "ManufacturingOpenSearchCollectionEndpoint",
        #     value=manufacturing_collection.attr_collection_endpoint,
        #     description="Endpoint of the Manufacturing OpenSearch Serverless collection"
        # )

        # CfnOutput(
        #     self,
        #     "ManufacturingKnowledgeBaseId",
        #     value=manufacturing_kb.attr_knowledge_base_id,
        #     description="ID of the Manufacturing Knowledge Base"
        # )

        # CfnOutput(
        #     self,
        #     "ManufacturingDataSourceId",
        #     value=manufacturing_kb.data_source_id,
        #     description="ID of the Manufacturing Data Source"
        # )
        ...
        # CfnOutput(
        #     self,
        #     "ManufacturingIndexWaiterFunctionArn",
        #     value=manufacturing_index_waiter_function.function_arn,
        #     description="ARN of the Manufacturing Index Waiter Lambda function"
        # )

        # CfnOutput(
        #     self,
        #     "ManufacturingIndexWaiterStatus",
